...
author: darius
---
I recently came across [EUMETSAT](https://navigator.eumetsat.int/product/EO:EUM:DAT:MSG:RIIE)
Regional Instability Index dataset which is shipped in the less known bufr format.
In this tutorial I am going to show how you can use Python to translate
_.bufr_ files to _.tiff_ files.
Besides the gdal library for writting, we will also need the [pyresample](https://pyresample.readthedocs.io/en/latest/)
and the [pybufr_ecmwf](https://github.com/jdkloe/pybufr-ecmwf) libraries.
_pyresample_ currently does not support the _.bufr_ format natively, however, it is 
...
to be used by end users. Within a _bufr_ file we find a number of _messages_ with
...
we are interested in writting a _tiff_ as output, the datasets of latitude and 
...
With this loop, we obtained all the necessary data to create a _tiff_ file. We have
...
algorithm and use our defined variables about the location to create a _tiff_ file 
as output.
